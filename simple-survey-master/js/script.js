...
function nextQuestion(question) {
	...
	if(question.value == "remote") {

		document.getElementById('checklist').style.display = 'block';
		
	} else if(question.value == "hybrid") {

		var divv = document.createElement('div');

		// question text
		var para = document.createElement('p');
		var text = document.createTextNode('Which office are you in?');
		para.appendChild(text);
		
		// select option
		var select = document.createElement('select');
		select.setAttribute("onchange", "nextQuestion(this)");

		let offices = new Array('Select Value', 'Portsmouth', 'Dover', 'Seattle', 'Boston');

		for(i=0; i<offices.length; i++) {

			var option = document.createElement("option");
			option.setAttribute("value", offices[i]);
		
			var optText = document.createTextNode(offices[i]);
	...
	} else if(question.value == "Portsmouth" || question.value == "Dover" || question.value == "Seattle" || question.value == "Boston") {

		var divv = document.createElement('div');

		// question text
		var para = document.createElement('p');
		var text = document.createTextNode('Do you need a badge?');
		para.appendChild(text);
		...
		let badge = new Array('Select Value', 'Yes', 'No');

		for(i=0; i<badge.length; i++) {

			var option = document.createElement("option");
			option.setAttribute("value", badge[i]);
		
			var optText = document.createTextNode(badge[i]);
			option.appendChild(optText);
			select.appendChild(option);
	...
	} else if(question.value == "No" || question.value == "Yes") {
		if(document.getElementById('employeeType').value == "intern" ) {

			var divv = document.createElement('div');

			// question text
			var para = document.createElement('p');
			var text = document.createTextNode('Do you want a temporary (Hotelr) desk or a permanent desk?');
			para.appendChild(text);
			...
			let desk = new Array('Select Value', 'Hotelr', 'Permanent');

			for(i=0; i<desk.length; i++) {

				var option = document.createElement("option");
				option.setAttribute("value", desk[i]);
			
				var optText = document.createTextNode(desk[i]);
				option.appendChild(optText);
				select.appendChild(option);
